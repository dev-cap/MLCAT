...
class CheckHeaders(object):
	...
	def check_validity(self, check_unavailable_uid='False', json_header_filename='headers.json'):
		...
		with open(json_header_filename, 'r') as json_file:
			...
			for chunk in lines_per_n(json_file, 9):
				...
				# Check if it is a mail that is sent directly to "lkml.subscriber@gmail.com", in which case it has not been
	...
	def add_missing_headers(self,to_add=missing_uid, unwanted_uid_filename="unwanted_uid.txt", uid_map_filename="thread_uid_map.json"):
		"""
		...
		with open(unwanted_uid_filename, 'r') as unw_file:
			for line in unw_file:
				self.unwanted_uid.add(int(line.strip()))
		to_add = [x for x in to_add if x not in self.unwanted_uid]
		# To prevent attempts to replace mails are known to be not available in the IMAP server:
		to_add = [x for x in to_add if x not in self.unavailable_uid]
		if len(to_add) > 0:
