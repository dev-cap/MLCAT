...
class TestCheckHeaders(object):	
	...
	def test_check_validity(self,mock_function):
		...
		mock_function.return_value.uid.return_value=(1,['5'])
		checkHeaders=CheckHeaders()
		assert checkHeaders.check_validity(True, self.in_headers_file)==5
	

	def test_remove_unwanted_headers(self):

		checkHeaders=CheckHeaders()
		checkHeaders.check_validity(False,self.in_headers_file)
		checkHeaders.remove_unwanted_headers(checkHeaders.unwanted_uid,self.in_headers_file,self.out_headers_file)
		with open(self.out_headers_file, 'r') as json_file:
	...
	def test_remove_duplicate_headers(self):
		...
		checkHeaders=CheckHeaders()
		checkHeaders.check_validity(False, self.in_headers_file)
		checkHeaders.remove_duplicate_headers(checkHeaders.duplicate_uid, self.in_headers_file,self.out_headers_file)	
		count_uid=0
		with open(self.out_headers_file, 'r') as json_file:
			for chunk in lines_per_n(json_file, 9):
				json_obj = json.loads(chunk)
	...
	def test_replace_invalid_headers(self,mock_function):
		...
		checkHeaders=CheckHeaders()
		checkHeaders.check_validity(False, self.in_headers_file)
		checkHeaders.replace_invalid_headers(checkHeaders.invalid_uid, self.in_headers_file, self.out_headers_file, self.unwanted_uid_file, self.uid_map_file)
