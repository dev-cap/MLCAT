    ...
    for year in range(2015, 2017):
        for month in ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']:
            if month in {'Jan', 'Mar', 'May', 'Jul', 'Aug', 'Oct', 'Dec'}:
                ...
                max_day = 30
            outstr = conversation_refresh_times(path_ob.headers_filename, path_ob.nodelist_filename, path_ob.edgelist_filename,
            ...
            if outstr is None:
                (a, b, c), rmsd = generate_crt_curve_fits(path_ob.foldername + '/curve_fit/' + month + '_' + str(year) + '/')
        ...
        if outstr is None:
            (a, b, c), rmsd = generate_crt_curve_fits(path_ob.foldername + '/curve_fit/' + 'FULL_' + str(year) + '/')
            yearly_curve_fit_coeffs.append((year, a, b, c, rmsd))

    with open(path_ob.foldername + '/curve_fit/' + 'crt_curve_fit_coefficients.csv', 'w') as csv_file:
